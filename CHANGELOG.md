...
### Fixed
- TOCTOU race in `doUpdateVisibleCells`: execution-active guard was only checked before the async `getParagraphInfo` HTTP call but not re-checked inside the `_cellStatusUpdateMutex` callback, allowing a false `syncConflict` to be set on cells that started executing during the HTTP round-trip.
- `doUpdateVisibleCells` only processing cells in the first visible range: the bounds check hardcoded `visibleRanges[0]` instead of using the current loop variable `range`, causing cells in secondary visible ranges (e.g. split notebook views) to be silently skipped.
- Unawaited `runExclusive` promise in `_dispatchInterpreter`: the return value of the interpreter-queue mutex call was dropped, turning any error from `_doExecutionSync` in "by interpreter" concurrency mode into an unhandled promise rejection.
- Stale `cell.metadata.status` check in `_doExecutionAsync`: used `cell.metadata.status === "PENDING"` instead of the fresh `paragraph.status === "PENDING"` from the server response, since `getParagraphInfo` queues metadata updates asynchronously via `pollUpdateCellMetadata`.
- Fire-and-forget `pollUpdateCellMetadata` in `getParagraphInfo`: unlike all other callsites (`runParagraph`, `updateParagraphText`, `updateParagraphConfig`), the call was not `await`ed, creating a window where callers see fresh paragraph data but `cell.metadata` still reflects old state.
