            ...
            (error) => {
                ...
                if (!error.response) {
                    ...
                    if (!isSilentOperation && !shouldSuppressError) {
                        if (isTimeoutError) {
                            window.showWarningMessage(`Connection timeout. Use "Refresh Notebook" to sync when connection is restored.`);
                        } else {
                            window.showWarningMessage(`Connection error. Use "Refresh Notebook" to sync when connection is restored.`);
                        }
                    ...
                    if (!isSilentOperation && !shouldSuppressError) {
                        window.showWarningMessage(`Gateway timeout. Use "Refresh Notebook" to sync when connection is restored.`);
                        recentErrors.set(errorKey, now);
                    }
                }
                    ...
                    if (!isSilentOperation && !shouldSuppressError) {
                        window.showWarningMessage(`Request failed: ${error.response.data?.message || error.response.statusText}. Try "Refresh Notebook" if issue persists.`);
                        recentErrors.set(errorKey, now);
                    }
                }
